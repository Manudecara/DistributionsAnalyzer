...
def page_load_and_fit():
    ...
    if file is not None:
        ...
        if rolling_analysis:
            ...
            dist_period = st.text_input('State distribution period', '1D')
            ...
            changing_period = st.text_input('State distribution change period', '1H')
            ...
            if rolling_timeseries:
                ...
                fig, ax = plt.subplots(1,1)
                ax.plot(df.rolling_variance)
                ax.set_title('Rolling Variance')
                plt.show()
                st.pyplot(fig)

                fig, ax = plt.subplots(1,1)
                ax.plot(df.rolling_skewness)
                ax.set_title('Rolling Skew')
                plt.show()
                st.pyplot(fig)
                
                fig, ax = plt.subplots(1,1)
                ax.plot(df.rolling_kurtosis)
        ...
        if reconstruction_analysis:  
            ...
            changing_period = st.text_input('A new distribution will be plotted every', '1H')
            small_index_groups = df.resample(changing_period)
            small_index_groups = list(small_index_groups.groups)
            small_rolling = len(df.loc[small_index_groups[0]:small_index_groups[1]]) - 1
            
            reconstruction = st.number_input('Reconstruct distribution after every x number of values', 2)
            ...
            if reconstruction_plots:
                ...
                for i in range(0, len(df), small_rolling): 
                    if i != 0:
                        ...
                        plt.show()
                        df_plot = pd.DataFrame(data = skews, index = dates)
                        df_plot['skews'] = df_plot[0]
                        df_plot['kurtoses'] = kurtoses
                        df_plot = df_plot[['skews', 'kurtoses']]
